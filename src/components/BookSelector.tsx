...
interface BookSelectorProps {
  mode: AppMode;
  onSelect: (slug: BookSlug) => void;
  onBack: () => void;
}

export default function BookSelector({ mode, onSelect, onBack }: BookSelectorProps) {
      ...
      <main className="container mx-auto px-4 sm:px-6 py-8 sm:py-16">
        <header className="text-center mb-10 sm:mb-16">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4">
            ...
            {isReadMode ? "Read Mode" : "Exam Mode"}
          </h1>
          <p className="text-base sm:text-lg text-slate-300 max-w-2xl mx-auto">
            {isReadMode
              ? "Choose a book to browse vocabulary with images"
        ...
        <div className="text-center mt-10 sm:mt-12">
          <button
            ...
            className="px-6 sm:px-8 py-3 bg-gradient-to-r from-pink-500 to-violet-500 text-white rounded-full font-semibold hover:from-pink-600 hover:to-violet-600 transition-all duration-300 hover:shadow-lg hover:shadow-pink-500/25 focus-visible:ring-2 focus-visible:ring-pink-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900"
          >
            Start with Starter
          </button>
        </div>
