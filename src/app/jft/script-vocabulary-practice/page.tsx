...
export default function ScriptVocabularyPracticePage() {
  const [authenticated, setAuthenticated] = useState(false);
  const [passwordInput, setPasswordInput] = useState("");
  const [passwordError, setPasswordError] = useState(false);

  const [currentPage, setCurrentPage] = useState(1);
  const [question, setQuestion] = useState<ScriptVocabQuestion | null>(null);
  ...
  useEffect(() => {
    ...
    const savedAnswers = localStorage.getItem("jft_script_vocab_answers");
    if (savedAnswers) {
      try {
  ...
  const fetchQuestion = useCallback(async (page: number) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/jft-script-vocabulary?page=${page}&limit=1`);
  ...
  const handleAnswer = (questionId: number, optionIndex: number) => {
    setAnswers((prev) => {
      ...
      localStorage.setItem("jft_script_vocab_answers", JSON.stringify(next));
      return next;
    });
  };
  ...
  const renderSentence = (q: ScriptVocabQuestion) => {
    if (q.type === "fill_blank") {
      const parts = q.sentence.split("（  ）");
      return (
        <p className="text-xl md:text-2xl text-white/90 leading-relaxed" style={{ fontFamily: "'Noto Sans JP', sans-serif" }}>
          {parts[0]}
          <span
            className="inline-block mx-1 px-4 py-1 rounded-lg font-bold text-lg"
            style={{
    ...
    if (q.type === "kanji_reading" && q.underlinedKanji) {
      const parts = q.sentence.split(q.underlinedKanji);
      return (
        <p className="text-xl md:text-2xl text-white/90 leading-relaxed" style={{ fontFamily: "'Noto Sans JP', sans-serif" }}>
          {parts[0]}
          <span
            className="inline-block mx-0.5 px-2 py-0.5 rounded-lg font-bold"
            style={{
              background: "rgba(139,92,246,0.25)",
              borderBottom: "3px solid rgba(139,92,246,0.6)",
              color: "#c4b5fd",
            }}
          >
            {q.underlinedKanji}
          </span>
          {parts.slice(1).join(q.underlinedKanji)}
        </p>
      );
    }
  ...
  const totalQuestions = pagination?.totalQuestions ?? 7;
  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        >
          <div className="absolute top-0 right-0 w-64 h-64 bg-linear-to-br from-amber-500/10 to-orange-500/10 rounded-full -mr-32 -mt-32 blur-3xl" />
          <div className="relative flex flex-wrap justify-between items-center">
            <div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span
                  className="px-3 py-0.5 rounded-full text-xs text-amber-300 font-bold"
                  style={{
        ...
        {loading && (
          <div className="text-center py-16">
            <div className="inline-block w-10 h-10 border-4 border-amber-500/30 border-t-amber-500 rounded-full animate-spin" />
          ...
          <div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <span
                  ...
                  style={{
                    background: question.type === "fill_blank"
                      ? "rgba(59,130,246,0.2)"
                      : "rgba(139,92,246,0.2)",
                    color: question.type === "fill_blank" ? "#93c5fd" : "#c4b5fd",
                    border: question.type === "fill_blank"
                      ? "1px solid rgba(59,130,246,0.3)"
                      : "1px solid rgba(139,92,246,0.3)",
                  }}
                >
                  {question.type === "fill_blank" ? "空欄補充" : "漢字読み"}
                </span>
            ...
            >
                ...
                <button
                  onClick={() =>
                    speakText(
                      ...
                      `sentence-${question.id}`
                    )
                  }
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all"
                  style={{
                    background:
                      speakingId === `sentence-${question.id}`
              ...
              >
                ...
                {showBn && question.sentence_bn && (
                  <p className="text-sm text-white/50 mt-3 pt-3" style={{ borderTop: "1px solid rgba(255,255,255,0.1)" }}>
                    {question.sentence_bn}
                  </p>
                )}
              </div>
            </div>
                ...
                {question.options.map((option, optIdx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {showBn && question.options_bn[optIdx] && (
                        <p className="text-xs text-white/40">
                          {question.options_bn[optIdx]}
                        </p>
                      )}
                    </button>
                  );
                ...
                <div className="mb-5">
                  ...
                  {showExplanation && (
                    ...
                    >
                      <div className="space-y-3">
                        <div>
                          <span className="text-xs font-bold text-amber-400 mb-1 block">📝 গ্রামার প্যাটার্ন:</span>
                          <p className="text-sm text-white/80 leading-relaxed">
                            {question.explanation}
                          </p>
                        </div>
                        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)" }} className="pt-3">
                          <span className="text-xs font-bold text-blue-400 mb-1 block">🇧🇩 বাংলায় ব্যাখ্যা:</span>
        ...
        >
          ইরোদোরি ভাই - JFT Script & Vocabulary
        </div>
      </div>
    </div>
  );
