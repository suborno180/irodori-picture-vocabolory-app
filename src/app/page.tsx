...
export type AppMode = "read" | "exam";

export default function Home() {
  const [mode, setMode] = useState<AppMode | null>(null);
  const [selectedBook, setSelectedBook] = useState<BookSlug | null>(null);

  const handleModeSelect = (selectedMode: AppMode) => {
    setMode(selectedMode);
  };

  const handleBookSelect = (slug: BookSlug) => {
  ...
  const handleSwitchMode = (newMode: AppMode, book: BookSlug) => {
    setMode(newMode);
    setSelectedBook(book);
  };

  if (selectedBook && mode === "read") {
    return (
      <ReadMode
  ...
  if (mode) {
    return <BookSelector mode={mode} onSelect={handleBookSelect} onBack={handleBack} />;
  }
      ...
      <main className="container mx-auto px-4 sm:px-6 py-12 sm:py-16">
        ...
        >
          ...
          <button
            ...
            aria-label="Enter Read Mode - Browse vocabulary lesson by lesson with images"
          >
            <div className="absolute top-0 left-0 h-1 w-full bg-green-500 opacity-80" />
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-green-500/20 flex items-center justify-center mb-3 sm:mb-4" aria-hidden="true">
              ...
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-green-500/20 flex items-center justify-center shrink-0" aria-hidden="true">
          ...
          <button
            ...
            aria-label="Enter Exam Mode - Test your knowledge with image-based quizzes"
          >
            <div className="absolute top-0 left-0 h-1 w-full bg-purple-500 opacity-80" />
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-purple-500/20 flex items-center justify-center mb-3 sm:mb-4" aria-hidden="true">
              ...
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-purple-500/20 flex items-center justify-center shrink-0" aria-hidden="true">
                <svg className="w-5 h-5 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
              </div>
